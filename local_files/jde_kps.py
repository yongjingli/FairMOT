...
class LoadImagesAndLabels:  # for training
    ...
    def get_data(self, img_path, label_path):
        ...
        if os.path.isfile(label_path):
            ...
            #
            kps = kps0.copy()
            kps[:, :, 0] = ratio * w * kps[:, :, 0] + padw
            kps[:, :, 1] = ratio * h * kps[:, :, 1] + padh
...
def random_affine(img, targets=None, kps=None, degrees=(-10, 10), translate=(.1, .1), scale=(.9, 1.1), shear=(-2, 2),
    ...
    if targets is not None:
        if len(targets) > 0:
            ...
            if len(kps) > 0:
                kps = kps[i]
                n_object, n_points, n_ = kps.shape
                for i in range(n_object):
                    for j in range(n_points):
                        if kps[i, j, 2] > 0:
                            kps[i, j, :2] = affine_transform(kps[i, j, :2], M)
...
class JointDatasetKps(LoadImagesAndLabels):  # for training
    default_resolution = [1088, 608]
    mean = None
    std = None
    num_classes = 1

    def __init__(self, opt, root, paths, img_size=(1088, 608), augment=False, transforms=None, num_joints=17):
        print('use JointDatasetKps')
        self.opt = opt
    ...
    def __getitem__(self, files_index):
        ...
        imgs, labels, keypoints, img_path, (input_h, input_w) = self.get_data(img_path, label_path)

        # return imgs, labels, kps, img_path, (input_h, input_w)
        ...
        ret = {'input': imgs, 'hm': hm, 'reg_mask': reg_mask, 'ind': ind, 'wh': wh, 'reg': reg, 'ids': ids, 'bbox': bbox_xys,
               'hps': kps, 'hps_mask': kps_mask, 'hp_offset': hp_offset, 'hp_ind': hp_ind, 'hp_mask': hp_mask, 'hm_hp': hm_hp}
        return ret
