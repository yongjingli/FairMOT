...
class opts(object):
  def __init__(self):
    ...
    self.parser.add_argument('--data_dir', type=str, default='/home/liyongjing/Egolee_2021/data/TrainData/jde_vggface2')
    ...
    self.parser.add_argument('--hp_offset_weight', type=float, default=1,
                             help='loss weight for keypoint local offsets.')
    self.parser.add_argument('--hp_wh_weight', type=float, default=1,
                             help='loss weight for keypoint local offsets.')
  ...
  def update_dataset_info_and_set_heads(self, opt, dataset):
    ...
    if opt.task == 'mot':
      ...
      opt.heads = {'hm': opt.num_classes,
                   ...
                   'hm_hp': key_points,
                   'hps': key_points*2,
                   'hp_wh': 2 if not opt.ltrb else 4}

      if opt.reg_offset:
        opt.heads.update({'reg': 2})
        opt.heads.update({'hp_offset': 2})

      # {'hm': 1, 'wh': 2, 'hps': 34, 'reg': 2, 'hm_hp': 17, 'hp_offset': 2}  # keypoints
      opt.nID = dataset.nID
      # opt.img_size = (1088, 608)
      opt.img_size = (864, 480)
      # opt.img_size = (576, 320)
